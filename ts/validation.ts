...
function validateForest<T>(objectId: string, forest: any, treeValidator: (tree: any) => tree is T): forest is T[] {
    if (!Array.isArray(forest))
        throw new ValidationErrors.UnexpectedTypeError(objectId, ["array"], typeof forest);

    for (let i = 0; i < forest.length; i++) {
        if (!treeValidator(forest[i]))
...
function validateString(objectId: string, candidate: any): candidate is string {
    ...
    if (candidate.length === 0)
        throw new Error(`The empty string is not a valid ${objectId}.`);

    return true;
}

function validateBoolean(objectId: string, candidate: any): candidate is boolean {
    if (typeof candidate !== "boolean")
        throw new ValidationErrors.UnexpectedTypeError(objectId, ["boolean"], typeof candidate);
...
function validateConfigType(candidate: any): candidate is ConfigType {
    if (typeof candidate !== "string")
        throw new ValidationErrors.UnexpectedTypeError("ConfigType", [...configTypes], typeof candidate);
...
function validateConfigMetaTree(candidate: any): candidate is ConfigMetaTree {
    function validateConfigMetaTree(candidate: any, isKey: boolean): candidate is ConfigMetaTree {
        ...
        if (!validateConfigType(candidate.type))
            throw new ValidationErrors.UnexpectedStateError();
        ...
        if (typeof candidate.default !== candidate.type)
            throw new ValidationErrors.UnexpectedTypeError("default", [candidate.type], typeof candidate.default);
        ...
        if ("validator" in candidate) {
            if (typeof candidate.validator !== "function")
                throw new ValidationErrors.UnexpectedTypeError("validator", ["function"], typeof candidate.validator);
            if (candidate.validator(candidate.default) !== true)
                throw new Error(`If a validator is given, defaults must be accepted; Default '${candidate.default}' was rejected.`);
        }

        // sub
        if ("sub" in candidate) {
            if (Array.isArray(candidate.sub)) {
                if (!validateForest<ConfigMetaTree>(
                    "sub",
                    candidate.sub,
                    (candidate): candidate is ConfigMetaTree => validateConfigMetaTree(candidate, false)
                ))
                    throw new ValidationErrors.UnexpectedStateError();
            } else {
                // Check non-keys don't have singleton subs
                if (!isKey)
                    throw new ValidationErrors.UnexpectedTypeError("sub", ["array"], typeof candidate.sub);
...
function validateConfigValueTree(candidate: any): candidate is ConfigValueTree {
    ...
    if ("sub" in candidate) {
        if (!Array.isArray(candidate.sub))
            throw new ValidationErrors.UnexpectedTypeError("valueForest", ["array"], typeof candidate.sub);
...
function validateCompliance(metaTree: ConfigMetaTree, valueForest: ConfigValueTree[], isFixed: boolean) {
    const validateCompliance = (metaTree: ConfigMetaTree, valueTree: ConfigValueTree) => {
        if (typeof valueTree.value !== metaTree.type)
            throw new ValidationErrors.UnexpectedTypeError("value", [metaTree.type], typeof valueTree.value);

        if ("validator" in metaTree)
            if (!metaTree.validator(valueTree.value))
                throw new Error(`If a validator is given, current values must be accepted; Value '${valueTree.value}' was rejected.`);
    ...
    const validateKeyLevelCompliance = (
        ...
        // Check non-empty
        if (valueForest.length === 0)
            throw new Error("valueForest length cannot be zero.");

        // Check for duplicate keys
        for (let i = 0; i < valueForest.length; i++)
            for (let j = i + 1, testeeKey = valueForest[i].value; j < valueForest.length; j++)
                if (testeeKey === valueForest[j].value)
                    throw new Error(`Key paths must be unique; Duplicate path ending with '${testeeKey}' was found.`);
    ...
    const validateValueLevelCompliance = (metaForest: ConfigMetaTree[], valueForest: ConfigValueTree[]) => {
        if (metaForest.length === 0)
            throw new Error("metaForest length cannot be zero.");
        if (valueForest.length === 0)
            throw new Error("valueForest length cannot be zero.");
        if (metaForest.length !== valueForest.length)
            throw new Error("valueForest length must match metaForest length when outside of key depth.");

        for (let i = 0, metaTree: ConfigMetaTree, valueTree: ConfigValueTree; i < metaForest.length; i++) {
    ...
    if (valueForest.length === 0) {
        if (isFixed)
            throw new Error("valueForest cannot be empty when isFixed is true.");
    } else {
        validateKeyLevelCompliance(metaTree, valueForest);
    }
}

export default function extractConfig(data: any): [string, number, ConfigMetaTree, ConfigValueTree[], boolean] {
    // Top level validation
    if (typeof data !== "object")
        throw new Error("data must be an object.");

    // Extract data
    const {title, metaTree, valueForest, isFixed} = data;

    // Individual validation
    if (!validateString("title", title))
        throw new ValidationErrors.UnexpectedStateError();
    if (!validateBoolean("isFixed", isFixed))
        throw new ValidationErrors.UnexpectedStateError();
    if (!validateConfigMetaTree(metaTree))
        throw new ValidationErrors.UnexpectedStateError();
    if (!validateForest("valueForest", valueForest, validateConfigValueTree))
        throw new ValidationErrors.UnexpectedStateError();

    // Group validation
    validateCompliance(metaTree, valueForest, isFixed);

    // Return valid config
    return [title, getKeyDepth(metaTree), metaTree, valueForest, isFixed];
}
